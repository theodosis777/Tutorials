...
class HorizontalCarousel extends Component {
  ...
  componentWillMount() {
    const img_width_as_percentage =
      window.innerWidth < styles.media.min_width
        ? 100
        : (styles.media.max_img_size / window.screen.availWidth) * 100;
  ...
  componentDidMount() {
    ...
    const img_width_in_pixels =
      this.card_container.children[0].getBoundingClientRect().width;

    let first_card_clone = this.card_container.children[0].cloneNode(true);
    let last_card_clone =
      this.card_container.children[
    ...
    window.addEventListener('resize', () => {
      ...
      for (let i = 0; i < this.card_container.children.length; i++) {
        this.card_container.children[
          i
        ].style.width = `${img_width_as_percentage}vw`;
      }
      ...
      this.card_container.style.transform = `translate(-${
        this.state.current_card * img_width_in_pixels
      }px)`;
    });
  }
  ...
  handle_next = () => {
    if (this.state.current_card < this.card_container.children.length - 1) {
      ...
      this.setState({ current_card: new_current_card }, () => {
        ...
        this.card_container.style.transform = `translate(-${
          img_width_in_pixels * this.state.current_card
        }px)`;
      ...
      this.setState({ current_card: new_current_card }, () => {
        ...
        if (this.state.current_card === 0) {
          setTimeout(() => {
            this.card_container.style.transitionDuration = '0.0s';
            this.card_container.style.transform = `translate(-${
              img_width_in_pixels * (this.card_container.children.length - 2)
            }px)`;
    ...
    return (
      <div>
        ...
        >
          ...
          >
            <Card
              card_number='http://picsum.photos/800/300'
              resize_width={this.state.current_img_width_as_percentage}
            />
            <Card
              card_number='http://picsum.photos/600/301'
              resize_width={this.state.current_img_width_as_percentage}
            />
            <Card
              card_number='http://picsum.photos/700/300'
              resize_width={this.state.current_img_width_as_percentage}
            />
            <Card
              card_number='http://picsum.photos/650/200'
              resize_width={this.state.current_img_width_as_percentage}
            />
          </div>
        </div>
